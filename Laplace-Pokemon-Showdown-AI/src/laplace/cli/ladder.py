...
class LadderEnginePlayer(EnginePlayer):
    """EnginePlayer that publishes only our wins to replay.pokemonshowdown.com."""

    async def _handle_battle_message(self, split_messages):
        for split_message in split_messages[1:]:
            if (
                len(split_message) >= 3
                and split_message[1] == "win"
                and to_id_str(split_message[2]) == to_id_str(self.username)
            ):
                battle_tag = split_messages[0][0].replace(">", "").strip()
                ...
                try:
                    ...
                    print(
                        f"    /savereplay sent for WIN: {battle_tag}",
                        flush=True,
                    )
                except Exception as exc:
                    print(
                        f"    WARNING: could not upload winning replay "
                        f"for {battle_tag}: {exc!r}",
                        flush=True,
                    )
...
def build_agent(account, fork=False, cls=LadderEnginePlayer, **extra):
...
def replay_url(battle_tag):
    """Return the correct Showdown replay URL, including private-room passwords."""
    battle_tag = battle_tag.replace(">", "").strip()

    if battle_tag.startswith("battle-"):
        battle_tag = battle_tag[len("battle-"):]

    return REPLAY_UPLOAD_URL + battle_tag
...
async def set_profile(agent):
    """Set the Showdown status after the account has successfully logged in."""
    if not STATUS:
        return

    try:
        await handle_threaded_coroutines(
            agent.ps_client.logged_in.wait(),
            agent.ps_client.loop,
        )
        await send_to_showdown(agent, f"/status {STATUS}")
        print(f"    status -> {STATUS}", flush=True)
    except Exception as exc:
        print(f"    WARNING: could not set status: {exc!r}", flush=True)
...
def save_battle(agent, tag, battle, saved, warned):
    """Write only won battles to the local replay archive."""
    if tag in saved:
        return False

    if not battle.finished or not battle.won:
        return False
    ...
    try:
        ...
        agent.save_replay(
            tag,
            os.path.join(REPLAY_DIR, f"{label}-{tag}.html"),
        )

        traces = getattr(agent, "traces", {}).get(tag)
        if traces:
            with open(
                os.path.join(REPLAY_DIR, f"{label}-{tag}.trace.json"),
                "w",
                encoding="utf-8",
            ) as f:
        ...
        if tag not in warned:
            warned.add(tag)
            print(
                f"  WARNING: could not save replay for {tag}: {exc!r}",
                flush=True,
            )
        return False
...
def sweep_replays(agent, saved, warned):
    """Save any won battle the poll loop missed before the agent is torn down."""
    n = sum(
    ...
    if n:
        print(
            f"  Saved {n} won replay(s) on exit -> {REPLAY_DIR}",
            flush=True,
        )
...
def check_elo_record(agent, tag, battle, run):
    ...
    if battle.won:
        try:
            ...
            if traces:
                with open(
                    os.path.join(ELO_DIR, f"elo{elo}-{tag}.trace.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
                    json.dump(traces, f, indent=1)
            ...
            print(
                f"    WARNING: could not archive the record replay: {exc!r}",
                flush=True,
            )
    try:
...
class Run:
    ...
    def __init__(self, total, track_elo=True, stop_on_record=True):
        self.total = total
...
async def main(default_format=DEFAULT_FORMAT):
    ...
    parser.add_argument("--until", default=None, metavar="HH:MM", help="stop after the current battle when local time reaches HH:MM")
    args = parser.parse_args()
    until_dt = None

    if args.until:
        try:
            target_time = datetime.strptime(args.until, "%H:%M").time()
        except ValueError:
            parser.error("--until must use HH:MM, e.g. 08:00")

        now = datetime.now()
        until_dt = datetime.combine(now.date(), target_time)

        if until_dt <= now:
            until_dt += timedelta(days=1)

        print(
            f"  Stop time: {until_dt.strftime('%Y-%m-%d %H:%M:%S')}",
            flush=True,
        )
    ...
    if args.no_stop_on_record:
        ...
        stop_on_record = True
    run = Run(args.battles, stop_on_record=stop_on_record)
    ...
    while run.done < run.total:
        if until_dt is not None and datetime.now() >= until_dt:
            print(
                f"Reached stop time {until_dt.strftime('%Y-%m-%d %H:%M:%S')}; "
                f"not starting another battle.",
                flush=True,
            )
            break
        remaining = run.total - run.done
        agent = build_agent(account, fork=FORK_MODE and not FORK_BASELINE)
        await set_profile(agent)
