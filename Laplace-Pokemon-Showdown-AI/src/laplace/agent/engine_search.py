...
from poke_env.player import Player, ForfeitBattleOrder
...
class EnginePlayer(Player):
    ...
    N_DETERMINIZATIONS = 8     # worlds per turn; more = steadier vote, less search each
    SEARCH_TIME_MS = 450       # MCTS budget per world
    THREADS = 8                # engine threads per world
    # Conservative automatic resignation.
    # This is a heuristic, not a mathematical proof of loss.
    RESIGN_ENABLED = True
    RESIGN_STREAK = 3
    RESIGN_LAST_MON_STREAK = 2
    RESIGN_MIN_WORLDS = 7
    RESIGN_TRIGGER_TOP_SHARE = 0.20
    RESIGN_VALUE_THRESHOLD = 0.10
    RESIGN_OPP_REPLIES = 3
    RESIGN_KO_THRESHOLD = 0.95
    ...
    def _resign_value_for_candidate(self, state, candidate, res):
        """Estimate our post-move win probability against strong opponent replies."""
        if self._value_model is None:
            return None

        opp_moves = sorted(
            [o for o in res.side_two
             if o.move_choice != "No Move"],
            key=lambda o: -o.visits,
        )[:self.RESIGN_OPP_REPLIES]
        ...
        for opp in opp_moves:
            try:
                branches = generate_instructions(
                    state,
                    self._engine_choice(candidate),
                    self._engine_choice(opp.move_choice),
                )
            except (KeyboardInterrupt, SystemExit):
                raise
            except BaseException:
            ...
            for branch in branches:
                if branch.percentage <= 0:
                    continue

                try:
                    next_state = state.apply_instructions(branch)
                    features = featurize(next_state, self.value_case_fix)
                    ...
                    weight = max(opp.visits, 1) * (branch.percentage / 100.0)
    ...
    def _no_viable_action(self, battle, pooled):
        """Return True only when essentially every action is losing across hidden worlds."""
        if not self._value_model:
            return False

        if len(self._worlds) < self.N_DETERMINIZATIONS:
            return False

        if not pooled:
            return False

        top_share = max(pooled.values(), default=1.0)
        ...
        # In ordinary positions, avoid the expensive resignation analysis
        # unless MCTS is already highly uncertain.
        #
        # In a last-mon/no-switch position, ALWAYS run it. A confident
        # MCTS argmax can still be a forced-loss move.
        if not last_mon and top_share > self.RESIGN_TRIGGER_TOP_SHARE:
            return False
        ...
        # Never resign if a revealed attack has a realistic immediate KO.
        for move in battle.available_moves:
            try:
                if estimate_damage_fraction(
                    me, move, opp, self.knowledge
                ) >= self.RESIGN_KO_THRESHOLD:
                    return False
            except (KeyboardInterrupt, SystemExit):
                raise
            except BaseException:
        ...
        # Build the actions worth checking.
        choices = []

        for choice, _score in sorted(
            pooled.items(),
            key=lambda kv: kv[1],
            reverse=True,
        ):
            if choice != "No Move" and choice not in choices:
                choices.append(choice)

        # Include legal switches even if MCTS gave them no visits.
        for mon in battle.available_switches:
            choice = f"switch {to_id_str(mon.species)}"
        ...
        for state, res in self._worlds:
            best_value = None

            for candidate in choices:
                value = self._resign_value_for_candidate(
                    state,
                    candidate,
                    res,
                )
            ...
            # Inability to evaluate means uncertainty, not defeat.
            if best_value is None:
                continue

            if best_value < self.RESIGN_VALUE_THRESHOLD:
                hopeless_worlds += 1

        return hopeless_worlds >= self.RESIGN_MIN_WORLDS

    def _should_resign(self, battle, pooled):
        """Forfeit only after repeated confirmation of an effectively lost position."""
        ...
        try:
            hopeless = self._no_viable_action(battle, pooled)
        except (KeyboardInterrupt, SystemExit):
            raise
        except BaseException:
            self.diag["resign_err"] = self.diag.get("resign_err", 0) + 1
            self._resign_streak[tag] = 0
            return False
        ...
        required = (
            self.RESIGN_LAST_MON_STREAK
            if last_mon
            else self.RESIGN_STREAK
        )

        streak = self._resign_streak.get(tag, 0) + 1
        self._resign_streak[tag] = streak

        print(
            f"[T{battle.turn}] resignation test: "
            f"hopeless=True, "
        ...
        self.diag["resign"] = self.diag.get("resign", 0) + 1

        print(
            f"[T{battle.turn}] RESIGNING: no viable action across "
            f"{len(self._worlds)} hidden worlds",
            flush=True,
        )
    ...
    def choose_move(self, battle):
        try:
            ...
            if self._should_resign(battle, pooled):
                elapsed_ms = (time.perf_counter() - t_turn) * 1000
                print(
                    f"[T{battle.turn}] FORFEIT | think time={elapsed_ms / 1000:.2f}s",
                    flush=True,
                )
            ...
            for choice, _share in ranked:
                ...
                if order is not None:
                    ...
                    self._emit(
                        ...
                        ranked=ranked,
                        mixed=mixed,
                        reorders=list(reorders),
                        fallback=False,
                        ms=elapsed_ms,
                    )

                    print(
                        f"[T{battle.turn}] move={choice} | "
                        f"think time={elapsed_ms / 1000:.2f}s",
                        flush=True,
                    )
            ...
            print(
                f"[T{battle.turn}] move=<fallback> | "
                f"think time={elapsed_ms / 1000:.2f}s",
                flush=True,
            )

            return (
                self.choose_max_damage_move(battle)
